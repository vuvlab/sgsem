...
##' This function carries out sgSEM
##'
##' sgSEM builds a network model of multiple continuous variables. Each pair of variables is tested for sensible paring relation chosen from 6 pre-selected common functional forms in linear regression settings. Adjusted R-squared is used for model selection for every pair.
##'
...
##' @param x A dataframe. By default it consideres all columns as stressors, except second column stores the system response variable.
##' @param stressor, by defult it consideres all columns as stressors except column number 2.
...
##' @return An object of class sgSEM, which is a list of the following items:
##'
##' \itemize{
...
sgSEM <- function(x,
  ...
  if(!missing(stressor)){
    ...
    neworder[-2] <- stressor.loc
    neworder[stressor.loc] <- 1
    x <- x[neworder]
  }else{
    stressor <- names(x)[-2]  ##It takes all columns as stressors except col2
  }
